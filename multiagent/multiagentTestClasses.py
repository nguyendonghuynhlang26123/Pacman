...
class MultiagentTreeState(object):
    ...
    def getScore(self):
        if VERBOSE:
            print(
                "getScore(%s) -> %s" % (self.state, self.problem.evaluation[self.state])
            )
    ...
    def isWin(self):
        if VERBOSE:
            print(
                "isWin(%s) -> %s" % (self.state, self.state in self.problem.winStates)
            )
    ...
    def isLose(self):
        if VERBOSE:
            print(
                "isLose(%s) -> %s" % (self.state, self.state in self.problem.loseStates)
            )
...
class MultiagentTreeProblem(object):
    def __init__(
        self, numAgents, startState, winStates, loseStates, successors, evaluation
    ):
...
class GradingAgent(Agent):
    def __init__(
        self, seed, studentAgent, optimalActions, altDepthActions, partialPlyBugActions
    ):
...
class PolyAgent(Agent):
    ...
    def construct_our_pacs(self, multiAgents, keyword_dict):
        pacs_without_stop = [
            multiAgents.StaffMultiAgentSearchAgent(**keyword_dict) for i in range(3)
        ]
...
class PacmanGameTreeTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        allActions = list(
            map(lambda x: json.loads(x), solutionDict["optimalActions"].split("\n"))
        )
        altDepthActions = list(
            map(lambda x: json.loads(x), solutionDict["altDepthActions"].split("\n"))
        )
        partialPlyBugActions = list(
            map(
...
class EvalAgentTest(testClasses.TestCase):
    def __init__(self, question, testDict):
        ...
        self.scoreMinimum = (
            int(testDict["scoreMinimum"]) if "scoreMinimum" in testDict else None
        )
        ...
        self.winsMinimum = (
            int(testDict["winsMinimum"]) if "winsMinimum" in testDict else None
        )
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        agentOpts = (
            pacman.parseAgentArgs(self.agentArgs) if self.agentArgs != "" else {}
        )

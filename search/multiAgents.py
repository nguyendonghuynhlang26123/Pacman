    ...
    def getAction(self, gameState):
        ...
        def expectimax(node, itercnt):
            ...
            if itercnt % agent_num != 0:  # Ghost node
                ...
                avgscore = sum(
                    [score / len(successorScoreList) for score in successorScoreList]
                )
                return avgscore
            else:  # Pacman max
                res = -1e10
...
class ExplorerAgent(ExpectimaxAgent):
    ...
    def exploreActions(self, position, legalActions):
        ...
        """
        directions = {a: Actions.getSuccessor(position, a) for a in legalActions}
        result = [k for k in directions.keys() if directions[k] not in self.explored]
    ...
    def getAction(self, gameState):
        ...
        legalActions = self.exploreActions(
            gameState.getPacmanPosition(), (gameState.getLegalActions(0))
        )
        ...
        if len(perceived_ghosts) == 0 and len(perceived_foods) == 0:
            if len(self.guide) > 0 or (
                gameState.getScore() < 0 and random.random() > self.miracle_rate
            ):
                print("MIRACLE")

...
def loadModuleFile(moduleName, filePath):
    spec = importlib.util.spec_from_file_location(moduleName, "%s.py" % moduleName)
...
def evaluate(
    ...
    for q in test_subdirs:
        ...
        tests = filter(
            lambda t: re.match(r"[^#~.].*\.test\Z", t), os.listdir(subdir_path)
        )
        ...
        def makefunc(question):
            return lambda grades: question.execute(grades)

        setattr(sys.modules[__name__], q, makefunc(question))
